...
public class GraphPanel2 extends JPanel {
    ...
    private Color lineColor2 = new Color(255, 0, 0, 180);
    private Color pointColor = new Color(100, 100, 100, 180);
    private Color gridColor = new Color(200, 200, 200, 200);
    private static final Stroke GRAPH_STROKE = new BasicStroke(2f);
    private int pointWidth = 4;
    private int numberYDivisions = 10;
    private double[] live,dead;
	private int size;
    public GraphPanel2(double[] live,double[] dead, int size) {
        this.live = live;
        this.dead = dead;
        this.size = size;
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            int y1 = (int) ((getMaxScore() - live[i]) * yScale + padding);
        ...
        for (int i = 0; i < size; i++) {
            int x1 = (int) (i * xScale + padding + labelPadding);
            int y1 = (int) ((getMaxScore() - dead[i]) * yScale + padding);
       ...
       for (int i = 0; i < graphPoints.size() -2; i++) {
        ...
        for (int i = 0; i < graphPoints2.size() -2; i++) {
...
//    @Override
    ...
    private double getMinScore() {
		...
		for (int i=0;i<size-1;++i) {
			minScore = Math.min(minScore, live[i]);
        }
        for (int i=0;i<size-1;++i) {
			minScore = Math.min(minScore, dead[i]);
        }
        return minScore;
    }
    ...
    private double getMaxScore() {
        ...
        for (int i=0;i<size-1;++i) {
            ...
            maxScore = Math.max(maxScore, live[i]);
        }
        for (int i=0;i<size-1;++i) {
			
            maxScore = Math.max(maxScore, dead[i]);
        }
        return maxScore;
    }
    ...
    public static void createAndShowGui(double[] live,double[] dead, int size) {
        JFrame frame = new JFrame("Vivas Rojas  Muertas Azul");
		JPanel panel = new JPanel();
		
		

		
			panel=(new GraphPanel2(live,dead,size));
